...
public class Envoltorio
{
	...
	List<String> texto = new ArrayList<String>(new Lec_Esc_File(fileName, "\\W+"));
        ...
        int n = Integer.parseInt(texto.get(0)); //Número de nodos
	boolean ady = Boolean.parseBoolean(texto.get(1));
	boolean dir = Boolean.parseBoolean(texto.get(2));
	int posicion = 3; 
	...
	*/
	...
	for(int i=0; i < n; ++i)
	    for(int j=0; j < n; ++j)
		{
		    if(i != j)
			{
			    this._arista_nueva_(i, j, Integer.parseInt(texto.get(posicion)), dir);}
			    posicion++;
